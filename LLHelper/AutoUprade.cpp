...
Logger autoUpgradeLogger("LLHelperUpgrade");
...
bool CheckAutoUpdate(bool isUpdateManually, bool forceUpdate)
{
    try
    {
        ...
        {
            autoUpgradeLogger.info("Your BDS does not match the current mainline LLtpa. Auto-update will not be pushed.");
            if (isUpdateManually)
                return false;
            else
                _endthreadex(0);
        ...
        else
        {
            ...
            Version verCurrent = LLHELPER_VERSION;
            ...
            autoUpgradeLogger.warn("Congratulations! The update download is complete. Restart the server now to enjoy the new version.");
        }
    }
