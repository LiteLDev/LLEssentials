...
bool DoCloseReq(decltype(reqs.begin()) rq, TPCloseReason res) {
	...
	if (playerA)
		playerA->sendTextPacket(res == TPCloseReason::deny ? tr("tpa.reason.deny") : tr("tpa.reason.timeout"),
			TextType::RAW);
	if (playerB)
		playerB->sendTextPacket(res == TPCloseReason::deny ? tr("tpa.reason.deny") : tr("tpa.reason.timeout"),
			TextType::RAW);
	reqs.erase(rq);
	return true;
}
...
public:
	...
	static void setup(CommandRegistry* registry) {
		...
		registry->registerCommand("tpa", "Teleport", CommandPermissionLevel::Any, { (CommandFlagValue)0 },
			{ (CommandFlagValue)0x80 });
		registry->addEnum<direction>("TPAOP2", { {"to",   direction::A_B},
												{"here", direction::B_A} });
		registry->addEnum<TpaCommand::TPAOP>("TPAOP", { {"ac",     TPAOP::ac},
													   {"de",     TPAOP::de},
													   {"cancel", TPAOP::cancel},
													   {"gui",    TPAOP::gui},
													   {"toggle", TPAOP::toggle} });
		registry->registerOverload<TpaCommand>("tpa", makeMandatory<CommandParameterDataType::ENUM>(&TpaCommand::dir,
			"direction",
			"TPAOP2",
			&TpaCommand::dir_isSet),
			makeMandatory(&TpaCommand::target, "player"));
		registry->registerOverload<TpaCommand>("tpa",
			makeMandatory<CommandParameterDataType::ENUM>(&TpaCommand::op, "op",
				"TPAOP",
				&TpaCommand::tpaop_isSet));
	}
};
...
public:
	...
	static void setup(CommandRegistry* registry) {
		using RegisterCommandHelper::makeMandatory;
		using RegisterCommandHelper::makeOptional;
		registry->registerCommand("warp", "Teleport", CommandPermissionLevel::Any, { (CommandFlagValue)0 },
			{ (CommandFlagValue)0x80 });
		registry->addEnum<WarpCommand::WARPOP>("WARPOP", { {"add", WARPOP::add},
														  {"del", WARPOP::del},
														  {"go",  WARPOP::go},
														  {"gui", WARPOP::gui},
														  {"ls",  WARPOP::ls} });
		registry->registerOverload<WarpCommand>("warp",
			makeMandatory<CommandParameterDataType::ENUM>(&WarpCommand::op, "op",
				"WARPOP"),
			makeOptional(&WarpCommand::val, "warp", &WarpCommand::val_isSet));
	}
};
...
public:
	void execute(CommandOrigin const& ori, CommandOutput& outp) const {
		...
		case HOMEOP::gui: {
			ServerPlayer* sp = ori.getPlayer();
			std::shared_ptr<Form::SimpleForm> form = std::make_shared<Form::SimpleForm>(tr("home.gui.title"),
				tr("home.gui.content"));
			for (auto& i : hm.data) {
	...
	static void setup(CommandRegistry* registry) {
		using RegisterCommandHelper::makeMandatory;
		using RegisterCommandHelper::makeOptional;
		registry->registerCommand("home", "Teleport", CommandPermissionLevel::Any, { (CommandFlagValue)0 },
			{ (CommandFlagValue)0x80 });
		registry->addEnum<HomeCommand::HOMEOP>("HOMEOP", { {"add", HOMEOP::add},
														  {"del", HOMEOP::del},
														  {"go",  HOMEOP::go},
														  {"gui", HOMEOP::gui},
														  {"ls",  HOMEOP::ls} });
		registry->registerOverload<HomeCommand>("home",
			makeMandatory<CommandParameterDataType::ENUM>(&HomeCommand::op, "op",
				"HOMEOP"),
			makeOptional(&HomeCommand::val, "home", &HomeCommand::val_isSet));
	}
};
...
public:
	...
	static void setup(CommandRegistry* registry) {
		using RegisterCommandHelper::makeMandatory;
		using RegisterCommandHelper::makeOptional;
		registry->registerCommand("back", "Back to last deathpoint", CommandPermissionLevel::Any,
...
public:
	...
	static void setup(CommandRegistry* registry) {
		using RegisterCommandHelper::makeMandatory;
		using RegisterCommandHelper::makeOptional;
		registry->registerCommand("suicide", "Goodbye", CommandPermissionLevel::Any, { (CommandFlagValue)0 },
			{ (CommandFlagValue)0x80 });
	...
	static void setup(CommandRegistry* registry) {
		...
		registry->registerOverload<LLtpaCommand>("lltpa",
			makeMandatory<CommandParameterDataType::ENUM>(&LLtpaCommand::op, "OP",
				"LLtpaOP"),
			makeOptional(&LLtpaCommand::isForce, "isForce",
